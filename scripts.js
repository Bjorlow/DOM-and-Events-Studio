...
//Code below ensures shuttle is pushed off the ground if user chooses for shuttle to take off
rocketPosY += 10;
...
document.addEventListener("click", function (event) {
    //This code below calculates the background image width that can be then observe on the page INspect console
    let bkgWidth = parseInt(window.getComputedStyle(shuttleBackground).getPropertyValue
    ('width'));
    console.log('background width is', bkgWidth);

    //all && line code below ensure that the rocket does not go over the image background borders
